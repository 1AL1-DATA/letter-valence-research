...
4. ~~Test on a held-out dataset~~ — **Done, for the cascade.** The cascade generalisation run (`src/benchmark_general.py`) evaluates the deployed architecture on a different labeled corpus (NewsMTSC general news); the letter formula itself has not been re-run on SST-2 / IMDB.
5. **Test on a non-English language** to see if the spectral signal transfers.

### 5.6 Cascade follow-up and cross-domain generalisation (2026-08-07)

Separate from the letter-formula result, this repository evaluates the **sentiment
cascade** used by the [esg-dashboard](https://github.com/1AL1-DATA/esg-dashboard) news
pipeline. The original three-tier design (DFT probe → letter RF → FinancialBERT) was
retired after benchmark evidence showed the letter tiers were strictly dominated: the
DFT probe never fired (max |v| = 0.922 < 0.95) and the letter RF fired on 3.6% of
sentences. The production cascade is now two tiers: a **word-level cheap tier**
(TF-IDF 1–2 grams + VADER + keyword features → 3-class logistic regression) that
decides when |v| ≥ 0.6, with FinancialBERT as the heavy fallback and VADER last-resort.

On the FinancialPhraseBank clear-polarity set (n = 1,967, 5-fold CV) the cascade
...
2021) — 5-coder-labelled 3-class sentiment from real-world general news (AllSides),
held-out `devtest_rw` split (n = 1,067). Two cheap-tier variants (trained only on
FinancialPhraseBank vs retrained on the 7,758 NewsMTSC train sentences) were combined
with two fixed heavy tiers (FinancialBERT vs a general-domain transformer,
`cardiffnlp/twitter-roberta-base-sentiment-latest`). Clear-polarity results (n = 651):
...
1. **The cheap word tier transfers out of finance.** Trained on nothing but
   FinancialPhraseBank, it beats the finance-tuned FinancialBERT on general news
   (0.4209 vs 0.3041, p ≈ 4×10⁻⁷) and closes most of the gap to VADER (still
   significantly below, p ≈ 5×10⁻⁴); retrained on news it is the strongest single
   non-transformer component (0.4931), not significantly different from VADER
   (p = 0.20).
...
- `src/benchmark_cascade.py` — 2-tier cascade eval (cheap word tier → heavy)
- `src/benchmark_general.py` — cross-domain cascade eval on general news (NewsMTSC)
- `src/figures_cascade.py` / `src/figures_general.py` — cascade figures
...
- `newsmtsc/` — NewsMTSC (EACL 2021) general-news sentiment: `train.jsonl` +
  `devtest_rw.jsonl` + dataset readme (Hamborg et al., MIT)
...
- `cascade_benchmark.json` / `cascade_predictions.csv` — FPB cascade eval
- `general_news_benchmark.json` / `general_news_predictions.csv` — NewsMTSC cascade eval
...
- `cascade_sentiment_eval.png` — FPB cascade figure (4 panels)
- `general_news_eval.png` — NewsMTSC cross-domain cascade figure (4 panels)
