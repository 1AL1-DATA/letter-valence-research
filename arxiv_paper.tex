...
Stratified random & $0.574 \pm 0.008$ & 0.693 & $-0.119$ \\
\midrule
...
\texttt{ahmedrachid/\allowbreak FinancialBERT-Sentiment-Analysis}) the cascade reaches
...
One caveat applies to the \emph{absolute} numbers here:
\texttt{ahmedrachid/\allowbreak FinancialBERT-Sentiment-Analysis} was itself fine-tuned on
FinancialPhraseBank (its model card reports fine-tuning on the dataset, with its
own 485-sentence test split), so the heavy tier already saw $\sim$90\% of our
4{,}846 evaluation sentences during fine-tuning; the in-domain accuracies are
therefore optimistic (in-sample) ceilings. The relative cascade-versus-heavy
comparison --- both systems share the identical heavy tier --- and the
...
\item \textbf{heavy\_fin} = \texttt{ahmedrachid/\allowbreak FinancialBERT-Sentiment-Analysis};
...
2. \textbf{The finance-tuned heavy is the domain-locked part.} FinancialBERT
    ...
    at 0.3041 (Wilson 95\% CI [0.270, 0.341]) --- well under both uniform random
    guessing (0.5 on this two-class clear set) and the majority-class baseline of
    0.616 (401 negative / 250 positive). With a
...
the clear set. We also checked for dataset leakage on the general-news side:
four of the 1,067 devtest sentences (two of them clear-polarity) also appear in
the NewsMTSC training split that trains the news cheap tier, a $\sim$0.4\%
overlap whose worst-case contribution to the +4.3-point headline is at most
$\sim$0.3 points. Effect sizes on general news are small, and the p-values are
