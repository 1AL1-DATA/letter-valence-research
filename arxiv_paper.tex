...
FinancialBERT (heavy\_fin) & 0.3041 & 0.4581 \\
Cheap tier trained on FPB only (cheap\_fpb) & 0.4209 & 0.5523 \\
VADER & 0.4685 & 0.5744 \\
Cheap tier trained on news (cheap\_news) & 0.4931 & 0.6069 \\
General BERT (heavy\_gen) & 0.5760 & 0.6641 \\
Cascade (cheap\_fpb $\rightarrow$ heavy\_gen) & 0.5975 & 0.6741 \\
\textbf{Cascade (cheap\_news $\rightarrow$ heavy\_gen)} & \textbf{0.6190} & \textbf{0.6940} \\
\bottomrule
\end{tabular}
\end{table}
...
1. \textbf{The cheap word tier transfers out of finance.} Trained on nothing but
   ...
   general news (0.4209 vs.\ 0.3041) and approaches VADER; retrained on news it
   is the strongest single non-transformer component (0.4931).
2. \textbf{The finance-tuned heavy is the domain-locked part.} FinancialBERT
   ...
   below chance (0.3041). With a domain-appropriate heavy, the cascade again
   beats heavy-only: \textbf{0.6190 vs.\ 0.5760} (exact McNemar $p = 10^{-6}$,
   significant at the 0.01 level), while the cheap tier absorbs 24.3\% of calls
   at 91.8\% accuracy. A threshold sweep (routing threshold $\in \{0.4,\dots,1.0\}$,
   band $\in \{0.05,0.1,0.15\}$, re-routed on stored per-instance valences)
   reaches 0.699 cascade accuracy at a 53\% heavy-tier share. The cascade
   \emph{approach} generalises; the heavy model must match the domain.

\paragraph{Borderline set (n = 416 neutral sentences).}
Table~\ref{tab:newsmt_border} reports the false-polarity rate (fraction of
genuinely neutral sentences labelled positive or negative). The cheap tiers are
the most polarity-prone components; routing them through either heavy tier
reduces false polarity on the FPB-trained variants but is roughly neutral on the
news-trained variant, mirroring the finance-only result that the cascade trades a
small polarity cost for a large compute saving.

\begin{table}[ht]
\centering
\caption{False-polarity rate on 416 held-out neutral NewsMTSC sentences (lower is better).}
...
\midrule
Keyword lexicon & 5.3\% \\
Heavy-only (heavy\_fin) & 15.4\% \\
General BERT (heavy\_gen) & 23.3\% \\
\textbf{Cascade (cheap\_news $\rightarrow$ heavy\_gen)} & \textbf{26.4\%} \\
Cheap tier trained on news (cheap\_news) & 26.7\% \\
Cascade (cheap\_fpb $\rightarrow$ heavy\_gen) & 27.4\% \\
Cheap tier trained on FPB only (cheap\_fpb) & 30.5\% \\
VADER & 36.5\% \\
\bottomrule
\end{tabular}
\end{table}
