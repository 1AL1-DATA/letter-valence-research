...
2. \textbf{The finance-tuned heavy is the domain-locked part.} FinancialBERT
   ...
   win: it beats heavy-only \textbf{0.6190 vs.\ 0.5760} (exact McNemar
     $p \approx 8\times10^{-7}$, significant at the 0.01 level), and both cascades
    beat their own cheap tiers ($p = 3\times10^{-9}$ and $p = 10^{-13}$). The
    \emph{FPB}-cheap cascade is numerically higher than heavy-only (0.5975) but this is
   \emph{not} significant (exact McNemar $p = 0.02$, above the
   multiple-comparison threshold), so it should be read as a point estimate,
   not an improvement. The cheap tier absorbs 24.3\% of clear calls
...
out of domain. Adjacent rates differ by less than the Wilson 95\% CI
($\pm$4--5 points on n = 416) and are not separable at this sample size; the
supported claims are the coarse ordering (keyword $\ll$ heavy\_fin $<$ general
heavy/cascades $<$ cheap tiers $<$ VADER) and the paired differences above.
...
supported claims are therefore limited to: the news-cheap cascade significantly
beats heavy-only on clear-polarity accuracy; the FPB-cheap cascade does not; the
cascade is not a false-polarity reducer on the general-news borderline set; and
the in-domain (FPB) false-polarity reduction is large and highly significant.
Effect sizes on general news are small, and the paired significance we report is
on fixed, overlapping samples; we encourage replication on additional corpora
before these cross-domain results are treated as stable.
