...
(macro-F1 0.9716): an exact McNemar test gives $p = 0.15$ (not significant), so
...
2. \textbf{The finance-tuned heavy is the domain-locked part.} FinancialBERT
   ...
   difference [2.8, 4.9] points), and both cascades beat their own cheap tiers
   ($p = 3\times10^{-9}$ and $p = 10^{-13}$). The \emph{FPB}-cheap cascade is
   numerically higher than heavy-only (0.5975) but this is \emph{not} significant
   ...
   variants do not differ reliably at the pre-specified $\alpha = 0.01$ level
   ($p = 0.049$; 95\% bootstrap CI on the difference [+0.2, +4.2] points is
   marginal). The cheap tier absorbs 24.3\% of clear calls
   (n = 158; Wilson 95\% CI [0.211, 0.277]) at 91.8\% accuracy (CI [0.864,
   0.951]). A threshold sweep (routing threshold $\in \{0.4,\dots,1.0\}$,
   band $\in \{0.05,0.1,0.15\}$, re-routed on stored per-instance valences)
   reaches 0.699 cascade accuracy at a 53\% heavy-tier share; this is the best
   point of an \emph{in-sample} grid, hence an upper bound rather than a
   held-out estimate. The cascade \emph{approach} generalises; the heavy model
...
keyword is significantly below heavy\_fin ($p = 7\times10^{-7}$), heavy\_fin
below heavy\_gen ($p = 2\times10^{-3}$), heavy\_gen below the two cascades
(13/0 and 17/0 one-directional discordant pairs, $p = 2\times10^{-4}$ /
$10^{-5}$), and cheap\_fpb below VADER ($p = 4\times10^{-3}$). The central
plateau --- heavy\_gen through the cheap tiers (23--31\%) --- is not otherwise
resolvable at n = 416: adjacent members differ by less than the paired
resolution of $\sim$7 points at $\alpha = 0.01$ / 80\% power (pairwise
$p \ge 0.25$), so no within-plateau ordering is supported; the cascade's step
above heavy\_gen is significant only because its 13 and 17 discordant pairs are
perfectly one-directional.

\begin{table}[ht]
\centering
\caption{False-polarity rate on 416 held-out neutral NewsMTSC sentences (lower is better), with Wilson 95\% CI. Rates within the CI of one another are not separable at n = 416.}
...
paired (exact McNemar) p-values are two-sided and pre-specified at
$\alpha = 0.01$ for the cascade-vs-baseline comparisons. The supported claims
...
bootstrap CI on the difference is [2.5, 6.3]); (ii) the \emph{FPB}-cheap
cascade does not ($p = 0.02$); (iii) the two cascade variants do not differ
reliably --- 0.6190 vs.\ 0.5975, $p = 0.049$, above the pre-specified
...
the 0--3-point gaps to its own cheap tier lie below the resolution of this
sample --- at n = 416 a paired test at $\alpha = 0.01$ / 80\% power resolves
only differences of $\sim$7 points --- whereas the 13/0 and 17/0 discordant
pairs behind the \emph{worst}-than-heavy 3.1/4.1-point gaps are significant
